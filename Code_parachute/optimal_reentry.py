...
def optimal_reentry(z, final_vz, max_force_drogue, max_force_main):
    z_deploy_drogue, z_deploy_main = z
    ...
    return alpha * np.abs(dynamics_obj.vz_vect[-1] - final_vz) + beta * np.abs(
        dynamics_obj.drogue.opening_force - max_force_drogue) + gamma * np.abs(
        dynamics_obj.mainpara.opening_force - max_force_main)
