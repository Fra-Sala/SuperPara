...
"""GUI to run the simulation. By Francesco Sala"""
...
def run_simulation():
    ...
    new_rocket = Rocket(float(cd0_rocket.get()), float(mass_payload.get()), float(cross_rocket.get()))  # cd0 rocket, mass of rocket, cross-section
    new_drogue = Hemisflo(float(z_deploy_drogue.get()), 0.2, 25)  # cd0 drogue, altitude of deployment, surface of chute, total porosity
    t_max = 500.0   # [s] maximum time for solution. The simulation will be stopped once the ground is reached
    x0 = 0.0
    dynamics_obj = DynamicsReentry(t_max, x0, float(z0.get())*1000, float(vx0.get()), float(vz0.get()), new_mainpara, new_drogue,
...
label_initial = ctk.CTkLabel(window, text = "DESIGN PARAMETERS")
label_initial.grid(row = 0, column = 2)
